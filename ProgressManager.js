...
class ProgressManager {
    ...
    getEmptyState() {
        return {
            ...
            in_progress_section: null, // Section đang được build dở dang { name, completed_elements: [] }
            last_updated: new Date().toISOString()
        };
    }

    /**
     * Đánh dấu bắt đầu build một section.
     */
    startSection(sectionName) {
        if (!this.data.in_progress_section || this.data.in_progress_section.name !== sectionName) {
            this.data.in_progress_section = {
                name: sectionName,
                completed_elements: []
            };
            this.save();
    ...
    /**
     * Ghi nhận một thẻ DOM đã được build thành công.
     */
    logElementBuilt(className, elementId) {
        if (this.data.in_progress_section) {
            this.data.in_progress_section.completed_elements.push({
                class: className,
                id: elementId,
    ...
    markSectionComplete(sectionName, elementId = null) {
        if (!this.isSectionDone(sectionName)) {
            ...
            // Xóa trạng thái in_progress khi đã hoàn thành
            if (this.data.in_progress_section && this.data.in_progress_section.name === sectionName) {
                this.data.in_progress_section = null;
            }
            this.save();
            return true;
        }
